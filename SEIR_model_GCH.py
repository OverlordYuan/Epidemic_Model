...
import pandas as pd
from config.g_config import N,beta,alpha,gamma_1,gamma_2,I_0,E_0,R_0,S_0,T
...
#         else:
#             if i-56<31:
#                 time.append("4月" + str(i - 56) + '日')
#             else:
#                 time.append("5月" + str(i - 86) + '日')
def funcSI(prop,_):
    Y = np.zeros(4)
    X = prop
    # 易感个体变化
...
# plt.title('全国（除湖北）(EG)疫情感染人数预测')
# xticks=list(range(0,len(time),3)) # 这里设置的是x轴点的位置
