from SEIR_model import RES as wu
from SEIR_model_HCW import RES as hu
from SEIR_model_GCH import RES as gu
...
Rate = list(map(lambda z:str(round(z*100,2))+ '%' if z<0 else "+"+str(round(z*100,2))+ '%',list(map(lambda x, y: x / y,  (np.array(pre_data) - np.array(real_data)).tolist(),real_data))))
data_hugu0 = pd.DataFrame(columns=head,index=index_0,data=[real_data,pre_data,Rate,(np.array(real_data) - np.array(real_data_1)).tolist(),(np.array(pre_data) - np.array(real_data_1)).tolist()])
...
workbook = xlrd.open_workbook('output/预测结果-'+get_day_of_day(0).strftime("%Y%m%d")+'.xls')  # 打开工作簿
...
new_workbook.save('output/预测结果-'+get_day_of_day(0).strftime("%Y%m%d")+'.xls')
