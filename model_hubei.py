...
ax.set_ylabel(r"湖北（除武汉）地区预计确诊人数（人）")
ax2.set_ylabel(r"湖北（除武汉）地区预计新增确诊人数（人）")
ax.set_xlabel("日期")

plt.title('湖北（除武汉）地区确诊人数预测')
...
plt.grid()
plt.savefig('picture/湖北（除武汉）地区疫情感染人数预测'+get_day_of_day(0).strftime("%m-%d")+'.png',bbox_inches='tight',dpi = fig.dpi)
# plt.show()
# plt.close()