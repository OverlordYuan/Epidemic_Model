...
import numpy as np
from config.g_config import N,beta,alpha,gamma_1,gamma_2,I_0,E_0,R_0,S_0,T,r0,u
...
for i in range(T):
    ...
    if i == 0:
        ...
        beta =R0[i-1]*gamma_2
        INI = (RES[-1][0], RES[-1][1], RES[-1][2], RES[-1][3], gamma_2,beta )
        temp = spi.odeint(funcSI_0,INI,T_range).tolist()[-1]
        RES.append(temp)
        # RES.add(spi.odeint(funcSI_0,INI, T_range))
NEW=[]
time = []
RES = np.array(RES)*N
temp = RES[:,1]
...
ax.grid()
ax.set_ylabel(r"全国（除湖北）预计确诊人数（人）")
ax2.set_ylabel(r"全国（除湖北）预计新增确诊人数（人）")
ax.set_xlabel("日期")

plt.title('全国（除湖北）确诊人数预测')
...
plt.savefig('picture/全国（除湖北）疫情感染人数预测'+get_day_of_day(0).strftime("%m-%d")+'.png',bbox_inches='tight',dpi = fig.dpi)
# plt.show()
# plt.close()